...
export const preparationApi: PreparationApi = {
  ...
  async detail(taskId, signal) {
    return read(
      await fetch(`/api/tasks/${encodeURIComponent(taskId)}/preparation`, {
        signal,
        cache: "no-store",
      }),
    );
  ...
  async command(value) {
    return read(
      await fetch(
        `/api/tasks/${encodeURIComponent(value.taskId)}/preparation/commands`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async upload(value, file) {
    return read(
      await fetch(
        `/api/tasks/${encodeURIComponent(value.taskId)}/documents/${value.role}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/octet-stream",
            "X-Symphony-Request-Id": value.requestId,
            "X-Symphony-Expected-Revision": String(value.expectedRevision),
            "X-Symphony-Filename": encodeURIComponent(value.filename),
          },
          body: file,
        },
      ),

...
function ArtifactPreviewContent({ taskId, artifact }: { taskId: string; artifact: ArtifactRef }) {
  ...
  return <li className="artifact-item">
    <a href={url} download>{artifact.id} · v{artifact.version}</a>
    <details className="artifact-digest">
      <summary aria-label={`Digest for ${artifact.id} v${artifact.version}`}>Digest</summary>
    ...
    {open && <>
      ...
      {!error && <pre className="artifact-preview">{text ?? "Loading artifact…"}</pre>}
    </>}
  </li>;
}

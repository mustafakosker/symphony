...
With the optional mock config enabled, open an issue, attach **Design document** and **Implementation plan**, and click **Prepare for ONA**. Review the deterministic prompt, select the repository and branch, and edit as needed. Text/target changes autosave after 500 ms idle; **Save draft** retries a failed save explicitly. Both required files accept UTF-8 `.md`/`.txt` only, 1 byte–1 MiB, with no blank content. Links, PDFs, and Word files are not accepted. Preview renders text literally, including HTML. Replacements create new immutable versions; removal only clears the current selection.

**Send to ONA** freezes the current Jira snapshot, saved prompt, document refs and target before launch. It is disabled during unsaved edits or unresolved upload errors. A failed replacement retains the old saved file and blocks send until a successful upload or **Discard failed replacement**. Leaving a task waits for document/draft writes and remains on the task after a failed save. Browser close/reload cannot guarantee an asynchronous save; heed the unsaved-changes warning.

Receipts are explicitly **Simulated ONA handoff**. This feature does not launch an environment, run Codex, or create a GitLab merge request. The future adapter must retain the same idempotency and lookup semantics. The existing generic coordinator still uses its normal runner and capability checks.

An uncertain timeout locks editing and new sends. Use **Check handoff status**, which looks up the original request; startup recovery also performs lookup only. Do not delete the local receipt ledger or manufacture a new request to bypass uncertainty. A definitive rejection unlocks editing: **Retry unchanged package** reuses the original request key, while **Send to ONA** creates a new package after revisions. Accepted packages are read-only and expose exact-version downloads.

Task events and document/prompt/package artifacts live in the workspace store. Mock receipt records and Jira last-sync state live under `localRoot/jira-handoff/`; keep that directory with the host's other durable local state. Corrupt/unreadable mock receipts remain unknown. Reconciliation never launches work. Shutdown aborts owned calls and persists uncertainty before releasing the host lock; persistence failures retain the lock for recovery. As with other artifacts, missing or changed document bytes prevent dispatch.

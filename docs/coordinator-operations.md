...
Back up the complete synced workspace **and** `localRoot/tasks/` together after stopping the coordinator, including `.intake`, events, artifacts, run output and local checkouts. Restore both roots to the same checkpoint on the designated host, verify configured paths and repository refs, then start and inspect recovery issues before approving retries. A OneDrive copy alone cannot restore live local work directories or prove that pending sync was uploaded. Never restore a stale snapshot over newer immutable events.

...
it('holds claimed filesystem bytes without creating a task, then resolves once across restart',async()=>{
 ...
 const intake=createIntake(f.workspace,store,0,{},{projectGate:gate});await intake.scan(1);await intake.scan(2);expect((await store.list()).tasks).toHaveLength(0);
 ...
 const reopened=createIntake(f.workspace,store,0,{},{projectGate:gate});await reopened.scan(4);expect((await store.list()).tasks).toEqual(first);
 }finally{await f.dispose();}
});

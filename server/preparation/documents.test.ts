...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), "jira-documents-"));
...
it("validates size and encoding without changing bytes", () => {
  ...
  expect(validateDocument("plan.txt", Buffer.alloc(1024 * 1024, 97)).size).toBe(
    1024 * 1024,
  );
});
it("replays after restart and preserves historical versions on replacement/removal", async () => {
  ...
  store = await openStore(root);
  service = createDocumentService(store, createPreparationOperations(store));
  task = await service.upload(command, bytes);
  expect(task.revision).toBe(2);
  expect(task.artifacts).toHaveLength(1);
  await expect(
    service.upload(command, Buffer.from("changed")),
  ).rejects.toThrow();
  task = await service.upload(
    { ...command, requestId: "replace", expectedRevision: task.revision },
    Buffer.from("replacement"),
  );
  expect(task.preparation!.documents.design!.ref.version).toBe(2);
  task = await service.remove({
    taskId: task.id,
    requestId: "remove",
    expectedRevision: task.revision,
    action: { kind: "remove-document", role: "design" },
  });
  expect(task.preparation!.documents.design).toBeNull();
...
it("does not select an upload if selection commit fails or a cancellation wins", async () => {
  ...
  store.apply = async () => {
    throw new Error("Before event commit");
  };
  await expect(
    service.upload(command, Buffer.from("Design")),
  ).rejects.toThrow();
  expect(
    (await store.get(command.taskId)).preparation!.documents.design,
  ).toBeNull();
  expect((await store.get(command.taskId)).artifacts).toHaveLength(0);
  store.apply = apply;
  await store.apply(command.taskId, 1, "cancel", {
    kind: "preparation",

...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), "jira-drafts-"));
...
it("saves exact user text and repeated preparation never regenerates it", async () => {
  ...
  const command = (
    kind: PreparationAction,
    expectedRevision: number,
    requestId: string,
  ) => ({ taskId: jiraTask().id, expectedRevision, requestId, action: kind });
  ...
  const save = command(
    {
      kind: "save",
      promptText: " Exact\n\nuser text.\r\n",
      target: { projectId: "app", repositoryId: "web", branch: "feature/x" },
    },
    task.revision,
    "save",
  );
  ...
  task = await drafts.prepare(
    command({ kind: "prepare" }, task.revision, "prepare-again"),
  );
  expect(task.artifacts).toHaveLength(artifactCount);
  expect((await drafts.detail(task.id)).promptText).toBe(
    " Exact\n\nuser text.\r\n",
  );
  ...
  await expect(
    drafts.save({
      ...save,
      action: { kind: "save", promptText: "changed", target: null },
    }),
  ).rejects.toThrow();
});
it("saves blank drafts but rejects unknown repositories and stale edits", async () => {
  ...
  const command = {
    ...
    expectedRevision: 1,
    requestId: "save",
    action: { kind: "save" as const, promptText: "", target: null },
  };
  ...
  await expect(
    drafts.save({ ...command, requestId: "stale" }),
  ).rejects.toThrow();
  await expect(
    drafts.save({
      ...command,
      ...
      action: {
        kind: "save",
        promptText: "x",
        target: { projectId: "app", repositoryId: "missing", branch: "main" },
      },
    }),
  ).rejects.toThrow();
});

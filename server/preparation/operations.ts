...
export type PreparationOperations = {
  ...
  replay(taskId: string, requestId: string, inputDigest: string): Promise<Task | null>;
  digest(value: unknown): string;
};
export function createPreparationOperations(store: Store): PreparationOperations {
  ...
  return { digest,
    serial<T>(taskId: string, work: () => Promise<T>): Promise<T> {
      ...
      void barrier.then(() => { if (queues.get(taskId) === barrier) queues.delete(taskId); });
      return result;
    },
    async replay(taskId, requestId, inputDigest) {
      const prior = await store.operation(taskId, requestId);
      if (!prior) return null;
      if (prior.event.kind !== 'preparation' || prior.event.inputDigest !== inputDigest)
        throw new BoundaryError('conflict', 'Request ID was used with different content');
...
export function assertEditable(task: Task, expectedRevision: number): void {
  ...
  if (task.revision !== expectedRevision) throw new BoundaryError('conflict', 'Task revision has changed');
  if (['done','cancelled','rejected'].includes(task.status)) throw new BoundaryError('conflict', 'Task is immutable');
  const status = task.preparation.attempts.at(-1)?.status;
  if (status === 'sending' || status === 'unconfirmed') throw new BoundaryError('conflict', 'Resolve the pending handoff first');
}
export function validateInput<T>(parser: () => T): T {
  try { return parser(); } catch (error) {
    if (error instanceof BoundaryError) throw error;
    throw new BoundaryError('invalid', error instanceof Error ? error.message : 'Invalid preparation request');
  }
}

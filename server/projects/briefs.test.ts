...
it("versions briefs immutably, replays saves, and preserves edits on stale writes", async () => {
  ...
  try {
    ...
    const input = {
      ...
      source,
      report: {
        format: "source-report-v1" as const,
        text: "Architecture and commands. Not run.",
        citations: [],
      },
    };
    const first = await briefs.save(input);
    expect(first.version).toBe(1);
    const second = await briefs.save({
      ...input,
      expectedVersion: 1,
      requestId: "second",
      author: "human-edited",
      report: { ...input.report, text: "Edited context" },
    });
    expect(second.version).toBe(2);
    expect(await briefs.save(input)).toEqual(first);
    await expect(
      briefs.save({ ...input, requestId: "stale" }),
    ).rejects.toMatchObject({ code: "conflict" });
    expect(await (await openBriefs(f.local, snapshots)).current(p.id)).toEqual(
      second,
    );
    expect(await briefs.get(p.id, 1)).toEqual(first);
    expect(await treeDigest(f.root)).toBe(before);
    await expect(
      briefs.save({ ...input, projectId: "other", requestId: "forged" }),
    ).rejects.toThrow();
  } finally {
    await f.dispose();
  }

...
async function setup(content = "First\n<script>Literal</script>\nThird\n") {
  ...
  await createRepository(f.root, "shop", { "source.ts": content });
  const root = {
    projectsRoot: f.root,
    generation: "g1",
    revision: "r1",
  ...
  const snapshot = await snapshots.materialize(p, resolved),
    entry = (await snapshots.verify(snapshot)).entries[0];
...
it("validates immutable file identity and returns literal bounded source lines", async () => {
  const f = await setup();
  await validateSourceReport(f.report, [f.snapshot], f.snapshots, {
    textBytes: 131072,
    citations: 256,
  });
  expect(
    await readCitation(f.report, "source", [f.snapshot], f.snapshots),
  ).toMatchObject({
    firstLine: 1,
    startLine: 2,
    endLine: 2,
    lines: ["First", "<script>Literal</script>", "Third"],
  });
});
it.each([
  { path: "../secret" },
  ...
  { endLine: 1003 },
  { startLine: 2.5 },
])("rejects invalid or unavailable citation %j", async (change) => {
  const f = await setup();
  f.report.citations = [{ ...f.citation, ...change }];
  await expect(
    validateSourceReport(f.report, [f.snapshot], f.snapshots, {
      textBytes: 131072,
      citations: 256,
    }),
  ).rejects.toThrow();
...
it("rejects unknown markers and citation IDs without accepting arbitrary paths", async () => {
  const f = await setup();
  await expect(
    readCitation(f.report, "../../source.ts", [f.snapshot], f.snapshots),
  ).rejects.toThrow();
  await expect(
    validateSourceReport(
      { ...f.report, text: "[cite:missing]" },
      [f.snapshot],
      f.snapshots,
      { textBytes: 131072, citations: 256 },
    ),
  ).rejects.toThrow();
  await expect(
    validateSourceReport(
      { ...f.report, citations: [f.citation, f.citation] },
      [f.snapshot],
      f.snapshots,
      { textBytes: 131072, citations: 256 },
    ),

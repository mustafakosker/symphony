...
it("discovers direct repositories without reading nested projects or mutating source metadata", async () => {
  ...
  await createRepository(join(f.root, "group"), "nested", {
    "README.md": "Nested\n",
  });
  await symlink(repo.path, join(f.root, "link"));
  ...
  expect(scan.entries.filter((e) => !e.error).map((e) => e.name)).toEqual([
    "shop",
  ]);
  expect(scan.entries.find((e) => e.name === "empty")).toMatchObject({
    canonicalPath: join(f.root, "empty"),
    observedCommit: null,
  });
...
it("rejects enclosing repositories, bare repositories, linked worktrees and external object alternates", async () => {
  ...
  await mkdir(join(f.root, "plain"));
  const repo = await createRepository(f.root, "real", { a: "A" });
  await git(f.root, ["init", "--bare", "bare"]);
  await git(repo.path, [
    "worktree",
    "add",
    "--detach",
    join(f.root, "worktree"),
  ]);
  await writeFile(
    join(repo.path, ".git/objects/info/alternates"),
    "/outside/objects\n",
  );
  const scan = await scanRoot(f.setting, { timeoutMs: 5000 });
  expect(scan.entries.filter((e) => !e.error)).toEqual([]);
  expect(scan.entries.find((e) => e.name === "real")?.error).toMatch(
    /alternat/i,
  );
});
it("rejects Git metadata symlinks and revalidates replaced source directories", async () => {
  ...
  await rm(join(repo.path, ".git/objects"), { recursive: true });
  await symlink(f.local, join(repo.path, ".git/objects"));
  await expect(validateSource(f.root, repo.path)).rejects.toThrow(/link/i);
  await rm(repo.path, { recursive: true });
  await symlink(f.local, repo.path);
...
it("distinguishes a missing root from a valid empty root", async () => {
  const f = await setup();
  expect((await scanRoot(f.setting, { timeoutMs: 5000 })).entries).toEqual([]);
  await rm(f.root, { recursive: true });
  await expect(scanRoot(f.setting, { timeoutMs: 5000 })).rejects.toThrow();
});

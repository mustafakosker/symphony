...
async function fixture() {
  ...
  let batch: JiraBatch = { complete: true, issues: [jiraIssue()] };
  let failed = false;
  let calls = 0;
  const options = {
    store,
  ...
  return {
    store,
    options,
    intake: createJiraIntake(options),
    set: (value: JiraBatch) => {
      batch = value;
    },
    fail: () => {
...
it("refreshes A/B/A source snapshots without replacing saved documents or original brief", async () => {
  ...
  expect(task.preparation!.target).toEqual({
    projectId: "app",
    repositoryId: "web",
    branch: "main",
  });
  const ref = await f.store.publishArtifact(
    task.id,
    "jira-design",
    Buffer.from("Design"),
  );
  await f.store.apply(task.id, task.revision, "upload", {
    kind: "preparation",
    inputDigest: "a".repeat(64),
    change: {
      kind: "document",
      role: "design",
      document: { role: "design", filename: "design.md", size: 6, ref },
    },
  });
  for (const description of ["changed", "Improve search latency."]) {
    f.set({
      complete: true,
      issues: [jiraIssue({ description, key: "NEW-1" })],
    });
    await f.intake.sync(now);
...
it("marks absence only after a complete successful snapshot and retains last success across restart", async () => {
  const f = await fixture();
  await f.intake.sync(new Date("2026-09-23T12:00:00Z"));
  f.set({ complete: false, issues: [] });
  await f.intake.sync(new Date("2026-09-23T12:01:00Z"));
  ...
  f.set({ complete: true, issues: [] });
  await restarted.sync(new Date("2026-09-23T12:03:00Z"));
  expect((await f.store.list()).tasks[0].preparation!.matchesQuery).toBe(false);
  f.fail();
  await restarted.sync(new Date("2026-09-23T12:04:00Z"));
...
it("coalesces refresh and waits 60 seconds between automatic attempts", async () => {
  ...
  await f.intake.tick(new Date(59000));
  expect(f.calls()).toBe(1);
  await f.intake.tick(new Date(60000));
  expect(f.calls()).toBe(2);
});
it("validates the complete batch before mutation and does not reimport cancelled issues", async () => {
  const f = await fixture();
  f.set({ complete: true, issues: [jiraIssue(), jiraIssue()] });
  await f.intake.sync(new Date());
  expect((await f.store.list()).tasks).toHaveLength(0);
  expect(f.intake.view().error).toBeTruthy();
  f.set({ complete: true, issues: [jiraIssue()] });
  await f.intake.sync(new Date());
  const task = (await f.store.list()).tasks[0];
  await f.store.apply(task.id, task.revision, "cancel", {
    kind: "preparation",
    inputDigest: "a".repeat(64),
    change: { kind: "cancel" },
  });
...
it("retries a partially applied batch without duplicates and requires selection for ambiguous mappings", async () => {
  const f = await fixture();
  f.options.registry.projects[0].repositories.push({
    id: "api",
    localPath: null,
    mcpProfile: "read",
    baseRef: "develop",
  ...
  f.set({
    complete: true,
    issues: [jiraIssue(), jiraIssue({ issueId: "10002", key: "APP-2" })],
  });
  const create = f.store.create.bind(f.store);
  let calls = 0;
  f.store.create = async (task, op) => {
    if (++calls === 2) throw new Error("Temporary write failure");
    return create(task, op);
  };
  await f.intake.sync(new Date());
  expect((await f.store.list()).tasks).toHaveLength(1);
  ...
  expect(
    (await f.store.list()).tasks.every((t) => t.preparation!.target === null),
  ).toBe(true);
});

...
beforeEach(async () => {
  ...
  server = createServer(
    createApi({
      store,
      intake: createIntake(root, store, 0),
      coordinator: {
        tick: async () => {},
        stopTask: async () => {},
        shutdown: async () => {},
      ...
      preparation,
      jira: { view: () => view, sync: async () => view, tick: async () => {} },
      targets: [],
    }),
  );
...
const headers = (overrides = {}) => ({
  ...
  "X-Symphony-Expected-Revision": "1",
  "X-Symphony-Filename": "design.md",
  ...overrides,
});
  ...
  for (const size of [1024 * 1024 + 1, 1024 * 1024]) {
    const status = await new Promise<number>((resolve, reject) => {
      ...
      for (let i = 0; i < size; i += 65536)
        req.write(Buffer.alloc(Math.min(65536, size - i), 97));
      req.end();
    });
    expect(status).toBe(size === 1024 * 1024 ? 200 : 413);
  }
  const task = await store.get(jiraTask().id);
  expect(task.preparation!.documents.design!.size).toBe(1024 * 1024);
});
it("validates origin, filename, roles and task binding before selecting bytes", async () => {
  for (const changes of [
    { Origin: "" },
    { Origin: "https://foreign.test" },
    { "X-Symphony-Filename": "%ZZ" },
    { "X-Symphony-Filename": "..%2Fdesign.md" },
  ]) {
    ...
    expect(res.status).toBe(changes.Origin !== undefined ? 403 : 400);
  }
  expect(
    (
      await fetch(uploadUrl().replace("/design", "/other"), {
        method: "POST",
        headers: headers(),
        body: "design",
      })
  ...
  const res = await fetch(
    `${base}/api/tasks/${jiraTask().id}/preparation/commands`,
    {
      method: "POST",
...
it("returns feature status and supports preparation and exact artifact downloads", async () => {
  ...
  const task = await response.json(),
    ref = task.preparation.documents.design.ref;
  const artifact = await fetch(
    `${base}/api/tasks/${task.id}/artifacts/${ref.id}?version=${ref.version}`,
  );
  expect(Buffer.from(await artifact.arrayBuffer())).toEqual(
    Buffer.from("\uFEFF# Café\r\n"),
  );
  expect(artifact.headers.get("Content-Security-Policy")).toContain("sandbox");
  expect(
    (await (await fetch(`${base}/api/workspace`)).json()).jira.simulated,
  ).toBe(true);
  expect((await fetch(`${base}/api/tasks/${task.id}/preparation`)).status).toBe(
    200,
  );
});

...
export type FileReviewIo = {
  ...
  readReceipt: typeof fileIo.readReceipt;
...
export function createFileReviews(options: FileReviewOptions): FileReviewAdapter {
  ...
  const { loadRecords: loadJournal, publishExclusive, readBounded, readReceipt, saveRecord: saveJournal, verifyMaterial } = {
    ...
    readReceipt: options.io?.readReceipt ?? fileIo.readReceipt,
    saveRecord: options.io?.saveRecord ?? fileIo.saveRecord,
    verifyMaterial: options.io?.verifyMaterial ?? fileIo.verifyMaterial,
  };
  async function loadRecords(root: string) {
    try { return await loadJournal(root); }
    catch (error) { throw new JournalFailure((error as Error).message, { cause: error }); }
  }
  async function saveRecord(root: string, record: RequestRecord) {
    try { await saveJournal(root, record); }
    catch (error) { throw new JournalFailure((error as Error).message, { cause: error }); }
  }
  ...
  async function submission(record: RequestRecord, nowMs: number): Promise<void> {
    ...
    if (record.phase === 'settled' && !record.receiptPublished) {
      ...
      const expected = Buffer.from(renderReceipt(record));
      try { await publishExclusive(options.workspaceRoot, receipt, expected); }
      catch (error) {
        if ((error as NodeJS.ErrnoException).code !== 'EEXIST') throw error;
        const existing = await readReceipt(options.workspaceRoot, receipt, expected.length);
        if (!existing || !same(existing, expected)) throw error;
      }
      await saveRecord(options.localRoot, { ...record, receiptPublished: true });
    }
  }
  async function prepareMaterials(record: RequestRecord): Promise<void> {
    for (const material of record.materials) {
      const bytes = await options.store.readArtifact(record.binding.taskId, material.ref);
  ...
  async function scan(nowMs: number): Promise<void> {
    ...
    for (const record of records) {
      ...
      try { await reconcile(record, issues); }
        ...
        issues.push(issue(record.token, 'publication', `Review export failed: ${(error as Error).message}`, record.binding.taskId));
        continue;
      }
      try { await submission(record, nowMs); }
      catch (error) {
        if (error instanceof JournalFailure) throw error;
        issues.push(issue(record.token, 'submission', `Review submission will retry: ${(error as Error).message}`, record.binding.taskId));
      }
      ...
      if (current.phase === 'settled' && current.outcome?.status === 'Needs correction' && !successors.has(current.token) && !loaded.issues.length) {
        try {
          ...
          if (error instanceof JournalFailure) throw error;
          issues.push(issue(current.token, 'correction', `Cannot issue correction: ${(error as Error).message}`, current.binding.taskId));
        }
      }
    }
    ...
    if (!loaded.issues.length) {
      ...
      for (const task of view.tasks) {
        ...
        try {
          ...
          if (error instanceof JournalFailure) throw error;
          issues.push(issue(task.id, 'export', `Cannot export review: ${(error as Error).message}`, task.id));
        }
      }
    }
  ...
  return { scan(nowMs) {
    const result = queue.then(async () => {
      try { await scan(nowMs); }
      catch (error) {
        if (!(error instanceof JournalFailure)) throw error;
        observations.clear();
        currentIssues = [issue('journal', 'infrastructure', `File review journal unavailable; file decisions suspended: ${error.message}`)];
      }
    });

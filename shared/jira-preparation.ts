...
export function preparationReady(value: JiraPreparation): boolean {
  return Boolean(value.documents.design && value.documents.implementation && value.prompt?.nonblank &&
    value.target?.projectId && value.target.repositoryId && value.target.branch.trim());
}

...
it.each([
  ...
  for (const match of result.matches)
    expect(match.text).toBe(text[match.field].slice(match.start, match.end));
});
it.each([
  "[missing] Task",
  "[] Task",
  ...
  "[shop][pay] Task",
  "[shop] [pay] Task",
])("blocks invalid target %s", (title) => {
  expect(
    resolveProjects({ title, description: "Payments" }, catalog).problems
      .length,
  ).toBeGreaterThan(0);
});
it("requires ambiguous names to be resolved and rejects stale choices", () => {
  const ambiguous = {
    ...
    projects: [
      ...catalog.projects,
      { id: "other", name: "other", aliases: ["Payments"] },
    ],
  };
  ...
  expect(
    resolveProjects(text, ambiguous, {
      excludedReferenceIds: [],
      ambiguities: { [key]: "pay" },
    }),
  ).toMatchObject({ referenceIds: ["pay"], problems: [] });
  expect(
    resolveProjects(text, ambiguous, {
      excludedReferenceIds: [],
      ambiguities: { stale: "pay" },
    }).problems.some((p) => p.code === "invalid-choice"),
  ).toBe(true);
});
it("excludes references but never the target", () => {
  expect(
    resolveProjects(
      { title: "[shop] Task", description: "Payments" },
      catalog,
      { excludedReferenceIds: ["pay"], ambiguities: {} },
    ),
  ).toMatchObject({ targetId: "shop", referenceIds: [], problems: [] });
  expect(
    resolveProjects({ title: "[shop] Task", description: "" }, catalog, {
      excludedReferenceIds: ["shop"],
      ambiguities: {},
    }).problems[0].code,
  ).toBe("invalid-choice");
});
it("takes the longest overlapping phrase and distinguishes unavailable roots", () => {
  const c = {
    ...
    projects: [
      ...catalog.projects,
      { id: "long", name: "shop platform", aliases: [] },
    ],
  };
  expect(
    resolveProjects({ title: "Task", description: "shop platform" }, c)
      .referenceIds,
  ).toEqual(["long"]);
  expect(
    resolveProjects(
      { title: "Task", description: "shop" },
      { ...catalog, state: "unavailable" },
    ).problems[0].code,
  ).toBe("root-unavailable");
});
it("derives titles without requiring frontmatter and uses stable choice ordering", () => {
  expect(deriveDraftText("\n# [shop] Task\n\nPayments", "other.md")).toEqual({
    title: "[shop] Task",
    description: "\nPayments",
  });
  expect(deriveDraftText("Details", "[shop] Task.md")).toEqual({
    title: "[shop] Task",
    description: "Details",
  });
  const d = { title: "x", description: "y" };
  expect(
    resolutionPayload(d, catalog, {
      excludedReferenceIds: ["a", "b"],
      ambiguities: { z: "a", x: "b" },
    }),
  ).toBe(
    resolutionPayload(d, catalog, {
      excludedReferenceIds: ["b", "a"],
      ambiguities: { x: "b", z: "a" },
    }),
  );
});
it("leaves prefix interpretation to legacy intake when no root is configured", () => {
  expect(
    resolveProjects(
      { title: "[legacy-project] Investigate", description: "shop" },
      { ...catalog, state: "unset" },
    ),
  ).toMatchObject({
    targetId: null,
